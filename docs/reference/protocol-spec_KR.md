...
| **커스텀/TCP** | **61 바이트** | **~249 KB** | **0.02%** |
| 커스텀/QUIC | ~81 바이트 | ~331 KB | 0.03% |

> **참고**: 커스텀 프로토콜 오버헤드는 13바이트 프레임 헤더 (prefix 4B + type 1B + length 4B + postfix 4B)와 48바이트 CHUNK_DATA 페이로드 헤더를 포함합니다.
...
│                    프로토콜 프레임 (v0.2)                        │
├────────────────────────────────────────────────────────────────┤
│ PREFIX (매직)     │ 4 바이트: 0x46545331 ("FTS1")               │
├────────────────────────────────────────────────────────────────┤
│ 메시지 타입       │ 1 바이트                                     │
├────────────────────────────────────────────────────────────────┤
...
오프셋  크기  필드
------  ----  -----
0       4     prefix (매직 넘버: 0x46545331 = "FTS1")
4       1     message_type
5       4     payload_length (빅엔디안)
9       N     payload
9+N     2     checksum (바이트 [0..9+N-1] 합계 mod 65536, 빅엔디안)
11+N    2     length_echo (payload_length의 하위 16비트, 빅엔디안)
```
...
│ 필드            │ 크기     │ 설명                                │
├─────────────────────────────────────────────────────────────────┤
│ checksum        │ 2 바이트 │ 선행 모든 바이트 합계 mod 65536     │
│ length_echo     │ 2 바이트 │ 페이로드 길이 (하위 16비트)         │
└─────────────────────────────────────────────────────────────────┘
...
수신자는 다음 순서로 프레임을 검증해야 합니다:

1. **Prefix 검사**: 처음 4바이트 == 0x46545331
2. **길이 타당성**: payload_length <= MAX_PAYLOAD_SIZE (기본값: 1MB + 48B 헤더)
3. **페이로드 읽기**: 정확히 payload_length 바이트 읽기
4. **체크섬 검증**: 계산된 체크섬 == 저장된 체크섬
5. **길이 에코 검증**: (payload_length & 0xFFFF) == length_echo

검사 실패 시 프레임을 버리고 다음 유효한 prefix를 스캔합니다.
...
│ 필드                 │ 크기     │ 설명                          │
├─────────────────────────────────────────────────────────────────┤
│ protocol_version     │ 4 바이트 │ 프로토콜 버전 (major.minor.   │
│                      │         │ patch.build)                   │
...
│ 필드                 │ 크기     │ 설명                          │
├─────────────────────────────────────────────────────────────────┤
│ protocol_version     │ 4 바이트 │ 합의된 프로토콜 버전          │
...
| 비트 | Hex 값   | 이름        | 설명                           |
|------|----------|-------------|--------------------------------|
| 0    | `0x01`   | first_chunk | 파일의 첫 번째 청크            |
| 1    | `0x02`   | last_chunk  | 파일의 마지막 청크             |
| 2    | `0x04`   | compressed  | 데이터가 LZ4 압축됨            |
| 3    | `0x08`   | encrypted   | 암호화 예약                    |
| 4-7  | -        | 예약됨      | 반드시 0                       |

**플래그 조합 (예시):**
```
0x00 = 플래그 없음 (중간 청크, 비압축)
0x01 = 첫 번째 청크만
0x02 = 마지막 청크만
0x03 = 단일 청크 파일 (첫 번째 + 마지막)
0x04 = 압축된 중간 청크
0x05 = 첫 번째 압축 청크 (첫 번째 + 압축)
0x06 = 마지막 압축 청크 (마지막 + 압축)
0x07 = 단일 압축 파일 (첫 번째 + 마지막 + 압축)
```
...
프로토콜은 시맨틱 버저닝과 정렬된 4바이트 버전 형식을 사용합니다:

```
┌─────────────────────────────────────────────────────────────────┐
│ 필드     │ 크기    │ 범위    │ 설명                              │
├─────────────────────────────────────────────────────────────────┤
│ Major    │ 1 바이트│ 0-255   │ 호환성 깨지는 변경                │
│ Minor    │ 1 바이트│ 0-255   │ 새 기능 (하위 호환)               │
│ Patch    │ 1 바이트│ 0-255   │ 버그 수정                         │
│ Build    │ 1 바이트│ 0-255   │ 빌드/리비전 번호                  │
└─────────────────────────────────────────────────────────────────┘

현재 버전: 0.2.0.0
와이어 인코딩: 0x00020000 (4 바이트, 빅엔디안)
```
...
|-----------|------|------|
| 0.2.x.x | 0.2.x.x | 호환 |
| 0.2.0.x | 0.2.1.x | 호환 (서버가 새 기능 보유) |
| 0.2.1.x | 0.2.0.x | 호환 (클라이언트 기능 다운그레이드) |
| 0.3.x.x | 0.2.x.x | 비호환 (pre-1.0에서 마이너 버전 불일치) |
| 1.x.x.x | 0.x.x.x | 비호환 (메이저 버전 불일치) |

> **참고**: pre-1.0 버전 (major=0)에서는 마이너 버전 변경에 호환성이 깨지는 변경이 포함될 수 있습니다. 1.0 릴리스 이후에는 메이저 버전 변경만 호환성을 깨뜨립니다.

### 버전 협상 전략

1. **클라이언트 요청**: 클라이언트는 CONNECT 메시지에 자신의 `protocol_version`을 보냅니다.
2. **서버 검증**: 서버는 클라이언트 버전과 자신의 버전을 비교합니다.
   - `client.major != server.major`인 경우: 연결 거부.
   - `client.minor > server.minor`인 경우: 허용하지만, 프로토콜을 `server.minor` 수준으로 다운그레이드합니다.
   - `client.minor < server.minor`인 경우: 허용, 서버는 하위 호환성을 지원합니다.
3. **응답**:
   - **성공**: 서버는 협상된 버전(`min(client.ver, server.ver)`)을 포함한 `CONNECT_ACK`를 보냅니다.
   - **실패**: 서버는 `incompatible_protocol_version` 사유와 함께 `UPLOAD_REJECT` (또는 전용 오류)를 보냅니다.

### 버전 불일치 처리

서버가 버전을 거부하는 경우, 정의되지 않은 동작을 방지하기 위해 거부 메시지를 보낸 후 즉시 연결을 닫아야 합니다.
