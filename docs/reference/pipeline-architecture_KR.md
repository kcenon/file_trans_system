...
> **참고:** 파이프라인 소멸자는 순환 참조로 인한 메모리 누수를 방지하기 위해 항상 모든 작업 큐를 정리하고 순환 참조를 명시적으로 끊습니다. 참조 순환 경로: 작업(job)이 `shared_ptr<pipeline_context>`를 보유 → context가 `shared_ptr<thread_pool>`을 보유 → thread_pool이 해당 작업들을 포함한 작업 큐를 보유. 소멸자는:
> 1. 스레드 풀의 작업 큐를 정리하여 대기 중인 작업을 제거
> 2. `context->thread_pool`을 reset하여 참조 순환을 끊음
> 3. 그 후 스레드 풀을 안전하게 종료
